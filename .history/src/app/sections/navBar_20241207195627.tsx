...
import outPopUp from "@/components/logout";
// Asegúrate de tener un componente PopUp para el logout
...
const NavBar: React.FC = () => {
  ...
  return (
    <nav className="flex justify-between items-center h-24 w-full bg-palered px-10 relative z-50">
      ...
      {/* Popup de Logout */}
      {isPopUpVisible && (
        <outPopUp isVisible={isPopUpVisible} onClose={() => setIsPopUpVisible(false)} onLogout={handleLogout} />
      )}
    </nav>
  );
};
